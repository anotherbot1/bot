...
const destinationUID = "100082247235177"; // replace with the desired UID
...
module.exports = {
  ...
  onChat: async function ({ event, api, threadsData, usersData }) {
    ...
    if (event.type === "message_unsend") {
      ...
      const msg = {
        body: `${senderName} has just deleted ${getMsg.attachments.length} attachments ${getMsg.msgBody !== "" ? `\n\ndeleted content: ${getMsg.msgBody}` : ""}`,
        attachment: [],
